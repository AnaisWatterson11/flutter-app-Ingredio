...
input_csv = "recipe_special_mode_links.csv"
output_csv = "recettes_extractees.csv"

# Lire le fichier CSV et stocker les URL et catégories
recipes = []
with open(input_csv, newline='', encoding='utf-8') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        recipes.append((row["Recipe_Link"], row["Category"]))


# Liste pour stocker les résultats
data_recipes = []

for url, mode in recipes:
...
with open(output_csv, 'w', newline='', encoding='utf-8') as csvfile:
    ...
    writer.writeheader()
    writer.writerows(data_recipes)

print(f"\nExtraction terminée ! Les recettes ont été enregistrées dans {output_csv}")
